echo "Woo country based product price compatibility"
current_dir="$PWD"
composer_run(){
...
copy_folder(){
  ...
  compressed_plugin_folder=$pack_folder"/woo-country-based-compatibility"
  if [ -d "$pack_folder"]; then
    rm -r "$pack_folder"
  fi
  mkdir "$pack_folder"
  mkdir "$compressed_plugin_folder"
  move_dir=("App" "vendor" "woo-country-based-compatibility.php")
...
function zip_folder(){
  ...
  zip_name="woo-country-based-compatibility"
  rm "$zip_name".zip
